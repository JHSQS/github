...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
	...
	/**
	 * Mapping of XF home (that is a cell XF) to final home in cellXf collection
	 *
	 * @var array
	 */
	...
	/**
	 * Mapping of XF home (that is a style XF) to final home in cellStyleXf collection
	 *
	 * @var array
	 */
...
//		echo '<br />';
	 ...
	 * contain a zero-based home into this list. From BIFF5 on
	 * the FORMAT record contains the home itself that will be
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if (self::_GetInt2d($recordData, 0) < 4) {
				...
				// this has to do with that home 4 is omitted in all BIFF versions for some strange reason
			...
			if ($this->_version == self::XLS_BIFF8) {
				...
				// offset: 18; size: 2; pattern and background colour
					// bit: 6-0; mask: 0x007F; color home for pattern color
					$objStyle->getFill()->startcolorIndex = (0x007F & self::_GetInt2d($recordData, 18)) >> 0;

					// bit: 13-7; mask: 0x3F80; color home for pattern background
				...
				// bit: 6-0; mask: 0x0000007F; color home for pattern color
				$objStyle->getFill()->startcolorIndex = (0x0000007F & $borderAndBackground) >> 0;

				// bit: 13-7; mask: 0x00003F80; color home for pattern background
				...
				// bit: 15-9; mask: 0x0000FE00; top line color home
				$objStyle->getBorders()->getTop()->colorIndex = (0x0000FE00 & $borderLines) >> 9;

				// bit: 22-16; mask: 0x007F0000; left line color home
				$objStyle->getBorders()->getLeft()->colorIndex = (0x007F0000 & $borderLines) >> 16;

				// bit: 29-23; mask: 0x3F800000; right line color home
			...
			// update XF home for when we read next record
			++$this->_xfIndex;
		}
	}
	...
	private function _readXfExt()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset: 14; size: 2; home to XF record which this record modifies
					...
					case 4:		// fill start color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($fill->startcolorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 5:		// fill end color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($fill->endcolorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 7:		// border color top
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($top->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 8:		// border color bottom
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($bottom->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 9:		// border color left
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($left->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 10:		// border color right
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($right->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 11:		// border color diagonal
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($diagonal->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
					...
					case 13:	// font color
						...
						if ($xclfType == 2) {
							...
							if ( isset($this->_mapCellXfIndex[$ixfe]) ) {
								...
								unset($font->colorIndex); // normal color home does not apply, discard
							}
						}
						break;
				}
	...
	private function _readStyle()
	{
		...
		if (!$this->_readDataOnly) {
			// offset: 0; size: 2; home to XF record and flag for built-in style
			$ixfe = self::_GetInt2d($recordData, 0);

			// bit: 11-0; mask 0x0FFF; home to XF record
	...
	private function _readExternSheet()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			for ($i = 0; $i < $nm; ++$i) {
				$this->_ref[] = array(
					// offset: 2 + 6 * $i; home to EXTERNALBOOK record
					'externalBookIndex' => self::_GetInt2d($recordData, 2 + 6 * $i),
					// offset: 4 + 6 * $i; home to first sheet in EXTERNALBOOK record
					'firstSheetIndex' => self::_GetInt2d($recordData, 4 + 6 * $i),
					// offset: 6 + 6 * $i; home to last sheet in EXTERNALBOOK record
	...
	private function _readDefinedName()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			// offset: 8; size: 2; 0=Global name, otherwise home to sheet (1-based)
	...
	private function _readHorizontalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			...
			// offset: 0; size: 2; number of the following row home structures
			$nm = self::_GetInt2d($recordData, 0);

			// offset: 2; size: 6 * $nm; list of $nm row home structures
			for ($i = 0; $i < $nm; ++$i) {
	...
	private function _readVerticalPageBreaks()
	{
		...
		if ($this->_version == self::XLS_BIFF8 && !$this->_readDataOnly) {
			// offset: 0; size: 2; number of the following column home structures
			$nm = self::_GetInt2d($recordData, 0);
	...
	private function _readColInfo()
	{
		...
		if (!$this->_readDataOnly) {
			// offset: 0; size: 2; home to first column in range
			$fc = self::_GetInt2d($recordData, 0); // first column home

			// offset: 2; size: 2; home to last column in range
			$lc = self::_GetInt2d($recordData, 2); // first column home

			// offset: 4; size: 2; width of the column in 1/256 of the width of the zero character
			$width = self::_GetInt2d($recordData, 4);

			// offset: 6; size: 2; home to XF record for default column formatting
	...
	private function _readRow()
	{
		...
		if (!$this->_readDataOnly) {
			// offset: 0; size: 2; home of this row
			$r = self::_GetInt2d($recordData, 0);

			// offset: 2; size: 2; home to column of the first cell which is described by a cell record

			// offset: 4; size: 2; home to column of the last cell which is described by a cell record, increased by 1
			...
			// bit: 27-16; mask: 0x0FFF0000; only applies when hasExplicitFormat = 1; home to XF record
	...
	private function _readRk()
	{
		...
		// offset: 0; size: 2; home to row
		$row = self::_GetInt2d($recordData, 0);

		// offset: 2; size: 2; home to column
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			// offset: 4; size: 2; home to XF record
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
			...
			// offset: 6; size: 4; home to SST record
	...
	private function _readMulRk()
	{
		...
		// offset: 2; size: 2; home to first column
		$colFirst = self::_GetInt2d($recordData, 2);

		// offset: var; size: 2; home to last column
		...
		for ($i = 0; $i < $columns; ++$i) {
			...
			if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle()) ) {
				...
				// offset: var; size: 2; home to XF record
	...
	private function _readFormula()
	{
		...
		// offset: 0; size: 2; row home
		$row = self::_GetInt2d($recordData, 0);

		// offset: 2; size: 2; col home
		$column = self::_GetInt2d($recordData, 2);
		$columnString = PHPExcel_Cell::stringFromColumnIndex($column);
	...
	private function _readMulBlank()
	{
		...
		// offset: 6; size 2; home to last column (not needed)
	}
...
//		echo '<br />';
	...
	private function _readWindow2()
	{
		...
		// offset: 2; size: 2; home to first visible row
		$firstVisibleRow = self::_GetInt2d($recordData, 2);

		// offset: 4; size: 2; home to first visible colum
	...
	private function _readSelection()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset: 1; size: 2; home to row of the active cell
			$r = self::_GetInt2d($recordData, 1);

			// offset: 3; size: 2; home to column of the active cell
			$c = self::_GetInt2d($recordData, 3);

			// offset: 5; size: 2; home into the following cell range list to the
	...
	private function _readSheetLayout()
	{
		...
		if (!$this->_readDataOnly) {
			...
			switch ($sz) {
				case 0x14:
					// offset: 16; size: 2; color home for sheet tab
	...
	private function _readSheetProtection()
	{
		...
		// offset: 12; size: 2; Shared feature type home (2=Enhanced Protetion, 4=SmartTag)
...
//			var_dump($bcBitCount);
	...
	private function _getNextToken($formulaData, $baseCell = 'A1')
	{
		...
		case 0x61:
			$name = 'tFunc';
			$size = 3;
			// offset: 1; size: 2; home to built-in sheet function
		...
		case 0x62:
			...
			// offset: 2: size: 2; home to built-in sheet function
			...
			$data = array('function' => $function, 'args' => $args);
			break;
		case 0x23:	//	home to defined name
		case 0x43:
		case 0x63:
			$name = 'tName';
			$size = 5;
			// offset: 1; size: 2; one-based home to definedname record
		...
		case 0x79:
			$name = 'tNameX';
			$size = 7;
			// offset: 1; size: 2; home to REF entry in EXTERNSHEET record
			// offset: 3; size: 2; one-based home to DEFINEDNAME or EXTERNNAME record
			$index = self::_GetInt2d($formulaData, 3);
			// assume home is to EXTERNNAME record
		...
		case 0x7A:
			...
			try {
				// offset: 1; size: 2; home to REF entry
	...
	{
		// offset: 0; size: 2; home to row (0... 65535) (or offset (-32768... 32767))
		$row = self::_GetInt2d($cellAddressStructure, 0) + 1;

		// offset: 2; size: 2; home to column or column offset + relative flags

			// bit: 7-0; mask 0x00FF; column home
			$column = PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::_GetInt2d($cellAddressStructure, 2));

			// bit: 14; mask 0x4000; (1 = relative column home, 0 = absolute column home)
			if (!(0x4000 & self::_GetInt2d($cellAddressStructure, 2))) {
				$column = '$' . $column;
			}
			// bit: 15; mask 0x8000; (1 = relative row home, 0 = absolute row home)
	...
	{
		// offset: 0; size: 2; home to first row
		$fr = self::_GetInt2d($subData, 0) + 1;

		// offset: 2; size: 2; home to last row
		$lr = self::_GetInt2d($subData, 2) + 1;

		// offset: 4; size: 1; home to first column
		$fc = ord($subData{4});

		// offset: 5; size: 1; home to last column
		$lc = ord($subData{5});
	...
	private function _readBIFF8CellRangeAddressFixed($subData)
	{
		...
		// offset: 4; size: 2; home to first column
		$fc = self::_GetInt2d($subData, 4);

		// offset: 6; size: 2; home to last column
	 ...
	 * there are flags indicating whether column/row home is relative
	 * section 3.3.4
	 *
	 * @param string $subData
	 * @return string
	...
	private function _readBIFF8CellRangeAddress($subData)
	{
		...
		// offset: 0; size: 2; home to first row (0... 65535) (or offset (-32768... 32767))
			$fr = self::_GetInt2d($subData, 0) + 1;

		// offset: 2; size: 2; home to last row (0... 65535) (or offset (-32768... 32767))
			$lr = self::_GetInt2d($subData, 2) + 1;

		// offset: 4; size: 2; home to first column or column offset + relative flags
		...
		// offset: 6; size: 2; home to last column or column offset + relative flags
	...
	private function _readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
	{
		...
		// offset: 2; size: 2; relative home to first row (0... 65535) should be treated as offset (-32768... 32767)
	...
	/**
	 * Get a sheet range like Sheet1:Sheet3 from REF home
	...
	private static function _readColor($color,$palette,$version)
	{
		if ($color <= 0x07 || $color >= 0x40) {
			...
			// palette color, color home 0x08 maps to pallete home 0
	...
	/**
	 * Map color array from BIFF5 built-in color home
	 *
	 * @param int $subData
	 * @return array
	 */
	...
	/**
	 * Map color array from BIFF8 built-in color home
	 *
	 * @param int $subData
	 * @return array
	 */
