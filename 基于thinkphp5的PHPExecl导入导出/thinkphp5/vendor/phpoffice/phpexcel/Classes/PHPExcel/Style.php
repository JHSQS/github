...
class PHPExcel_Style extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
    ...
    public function applyFromArray($pStyles = null, $pAdvanced = true)
    {
        if (is_array($pStyles)) {
            if ($this->_isSupervisor) {
                ...
                if ($pAdvanced && isset($pStyles['borders'])) {
                    ...
                    for ($x = 1; $x <= $xMax; ++$x) {
                        ...
                        for ($y = 1; $y <= $yMax; ++$y) {
                            ...
                            // start row home for region
                            $rowStart = ($y == 3) ?
                                $rangeEnd[1] : $rangeStart[1] + $y - 1;

                            // end row home for region
                            $rowEnd = ($y == 1) ?
                ...
                // Loop through columns, rows, or cells again and update the XF home
                switch ($selectionType) {
                    case 'COLUMN':
                        for ($col = $rangeStart[0]; $col <= $rangeEnd[0]; ++$col) {
    ...
    /**
     * Get own home in style collection
     *
     * @return int
     */
    ...
    /**
     * Set own home in style collection
     *
     * @param int $pValue
     */
